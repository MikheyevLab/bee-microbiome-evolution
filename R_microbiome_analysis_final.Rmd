...
ord2<- ps1
ord2 = subset_samples(ord2, treatment != "hive_bee" & treatment != "positive_control"& treatment != "start_microbiome" & cycle !="cycle_three_after_stress")
...
NMDS_main
ggsave("R_microbiome_figures/NMDS.png", height = 5, width = 10)
```

```{r PCoA, fig.heigt=8, fig.width=9}
ord2<- ps1
ord2 = subset_samples(ord2, treatment != "hive_bee" & treatment != "positive_control"& treatment != "start_microbiome" & cycle !="cycle_three_before_stress"& cycle !="cycle_three_after_stress")
ord2 <- transform_sample_counts(ord2, function(OTU) {OTU / sum(OTU)})
sample_data(ord2)$cycle<-factor(sample_data(ord2)$cycle,levels=c("cycle_one","cycle_two"), labels=c("Cycle one","Cycle two"))
levels(sample_data(ord2)$cycle)

sample_data(ord2)$treatment2<-factor(sample_data(ord2)$treatment2,levels=c("Control","Chlorothalonil","Glyphosate","Tetracycline"),labels=c("Control","Chlorothalonil","Glyphosate","Tetracycline"))
levels(sample_data(ord2)$treatment2)

set.seed(42)
dist<-phyloseq::distance(ord2, method="bray") 
ordination = ordinate(ord2, method="PCoA", distance=dist)
cat("stress is:", ordination$stress)
p2 <- plot_ordination(ord2, ordination, color="treatment2", shape="cycle") +geom_point(size=3.5)+scale_colour_manual(values=c("#55596a", "#6ebe9f","#f3a935", "#D45E79"))+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ theme(panel.border = element_blank())+ theme(panel.background = element_blank())+theme_classic()+ 
  theme(text = element_text(size = 17))+theme(axis.line = element_line(colour = "black")) + theme(legend.title = element_blank(),
                legend.background = element_blank(),
                legend.key = element_blank()) 
p2<-p2+stat_ellipse(aes(group = treatment2),size=1.1)
p2
ggsave("R_microbiome_figures/Supp_PCoA.png", height = 5, width = 10)
```
...
```{r taxa_plot}
ps2 <-taxa3
ps3 = subset_samples(ps2, cycle !="positive_control" & cycle !="hive_bee" & cycle !="cycle_three_after_stress"& cycle !="start_microbiome")
...
p<-p+ theme(legend.text=element_text(size=11, face = "italic"))+theme(legend.key = element_rect(color = NA, fill = NA),
                                                     legend.key.size = unit(0.6, "cm"))+theme(legend.title = element_blank())
p<-p + facet_grid( ~cycle, scales="free_x",space="free") +
  theme(strip.text = element_text(size=12,face="bold",color="grey40"))
taxa2<-p+theme(axis.title.y = element_text(size=11, face="bold"))+theme(axis.text.y = element_text(size=11, face="bold"))+theme(axis.text.x = element_text(size=10, face="bold", angle = 90))+theme(axis.title.x = element_blank())+theme(axis.text.x = element_text(vjust = 0.5,hjust=1))+theme(strip.background =element_rect(fill="white", color="white"))+theme(axis.text.x = element_text(vjust = 0.5,hjust=1))
taxa2
...
```{r, figure_1}
#text <- paste("Figure 2 Alpha diversity Shannon index (A), comparing control to treatments by Wilcoxon-Tests. NMDS on Bray-Curtis dissimilarity (B), ellipses represent 95% confidence intervals around treatment centroids. Stress of NMDS is: 0.09. Taxonomy of genera with at least 1% relative abundance (C).", sep = " ")
#text.p <- ggparagraph(text = text, face = "italic", size = 11, color = "black")
# Arrange the plots on the same page

#legend <- get_legend(taxa2)
#taxa3<-taxa2+theme(legend.position = "none")
#p<-ggarrange(Shannon2,taxa2,NMDS_main,legend, 
         # ncol = 2, nrow = 2,labels = c("A", "B","C"))

#try<-ggarrange(Shannon2,NMDS_main)


library(egg) #helps to correctly align the plots

figure_1 <- "R_microbiome_figures/manuscript_main.png"
png(figure_1, 7 * plot_res, 8 * plot_res, res = plot_res)
ggarrange(Shannon2, NMDS_main,nrow=2,labels = c("A", "B"))
invisible(dev.off())
knitr::include_graphics(figure_1, dpi = plot_res)
```

```{r, main_figure2}
figure_2 <- "R_microbiome_figures/main_figure2.png"
png(figure_2, 6 * plot_res, 3.9 * plot_res, res = plot_res)
p2<-ggarrange(taxa2,labels = "C")
invisible(dev.off())
knitr::include_graphics(figure_2, dpi = plot_res)
```

## Run statistics

### Treatments different to the control?
#### test difference between control and treatments to verify statistically difference seen in ordination
```{r}
set.seed(42)
compare=ps1
...
cycle1 <- transform_sample_counts(cycle1, function(OTU) {OTU / sum(OTU)})
...
cycle2 <- transform_sample_counts(cycle2, function(OTU) {OTU / sum(OTU)})
cycle2.1 <- as(sample_data(cycle2), "data.frame")

cycle3b = subset_samples(compare, cycle == "cycle_three_before_stress")
cycle3b <- prune_taxa(taxa_sums(cycle3b) > 0, cycle3b)
cycle3b <- transform_sample_counts(cycle3b, function(OTU) {OTU / sum(OTU)})
cycle3b.1 <- as(sample_data(cycle3b), "data.frame")

cycle3a = subset_samples(compare, cycle == "cycle_three_after_stress")
cycle3a <- prune_taxa(taxa_sums(cycle3a) > 0, cycle3a)
cycle3a <- transform_sample_counts(cycle3a, function(OTU) {OTU / sum(OTU)})
cycle3a.1 <- as(sample_data(cycle3a), "data.frame")

#cycle 1

subs <- subset_samples(cycle1, treatment2 %in% c("Control", "Chlorothalonil"))
metadata <- as(sample_data(subs), "data.frame")
adonis(distance(subs, method="bray") ~ treatment2,
...
subs <- subset_samples(cycle3_before_stress, treatment3 %in% c("Control", "Chlorothalonil"))
metadata <- as(sample_data(subs), "data.frame")
adonis(distance(subs, method="bray") ~ treatment3,
       data = metadata, perm=999)

subs <- subset_samples(cycle3_before_stress, treatment3 %in% c("Control", "Glyphosate"))
metadata <- as(sample_data(subs), "data.frame")
adonis(distance(subs, method="bray") ~ treatment3,
       data = metadata, perm=999)

subs <- subset_samples(cycle3_before_stress, treatment3 %in% c("Control", "Tetracycline"))
metadata <- as(sample_data(subs), "data.frame")
adonis(distance(subs, method="bray") ~ treatment3,
       data = metadata, perm=999)
...
subs <- subset_samples(compare1, treatment_cycle3 %in% c("control_Chlorothalonil_cycle_3_after_stress", "Chlorothalonil_cycle_3_after_stress"))
metadata <- as(sample_data(subs), "data.frame")
adonis(distance(subs, method="bray") ~ treatment_cycle3,
       data = metadata, perm=999)

subs <- subset_samples(cycle3a, treatment3 %in% c("control_Glyphosate", "Glyphosate"))
metadata <- as(sample_data(subs), "data.frame")
adonis(distance(subs, method="bray") ~ treatment3,
       data = metadata, perm=999)

subs <- subset_samples(cycle3a, treatment3 %in% c("control_Tetracycline", "Tetracycline"))
metadata <- as(sample_data(subs), "data.frame")
adonis(distance(subs, method="bray") ~ treatment3,
       data = metadata, perm=999)
```
Tetracycline is highly different to the control in all cycles which is obvious. Chlorothalonil does not show significant differences and Glyphosate is significantly different after cycle 1 and 3.

#### PERMANOVA to test general effects of treatment across cycles
```{r treatment_effects}
set.seed(42)
compare=ps1
all = subset_samples(compare, cycle == "cycle_one"| cycle == "cycle_two"| cycle == "cycle_three_before_stress"| cycle == "cycle_three_after_stress")
all <- prune_taxa(taxa_sums(all) > 0, all)
all <- transform_sample_counts(all, function(OTU) {OTU / sum(OTU)})
all.1 <- as(sample_data(all), "data.frame")
...
cycle3_before_stress <- transform_sample_counts(cycle3_before_stress, function(OTU) {OTU / sum(OTU)})
cycle3_before_stress.2 <- as(sample_data(cycle3_before_stress), "data.frame")

cycle3_after_stress = subset_samples(compare, cycle == "cycle_three_after_stress")
cycle3_after_stress <- prune_taxa(taxa_sums(cycle3_after_stress) > 0, cycle3_after_stress)
cycle3_after_stress <- transform_sample_counts(cycle3_after_stress, function(OTU) {OTU / sum(OTU)})
cycle3_after_stress.2 <- as(sample_data(cycle3_after_stress), "data.frame")

d = distance(all, "bray")
adonis(d ~ treatment3, all.1, perm=999)
...
d4 = distance(cycle3_after_stress, "bray")
adonis(d4 ~ treatment3, cycle3_after_stress.2, perm=999)
```
...
#cycle 2 control (p= 0.001) significant cage variation
...
#cycle 3 before stress only in Glyphosate treatment significant cage effects (p=0.006)
```

### Time effect
#### Test if treatments differ across cycle 1,2,3
...
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle,
       data = metadata, perm=999)

subs1 <- subset_samples(compare2, treatment_cycle %in% c("control_Glyphosate_cycle_3_before_stress", "control_Glyphosate_cycle_3_after_stress"))
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle,
       data = metadata, perm=999)

subs1 <- subset_samples(compare2, treatment_cycle %in% c("control_Tetracycline_cycle_3_before_stress", "control_Tetracycline_cycle_3_after_stress"))
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle,
       data = metadata, perm=999)
...
#### compare control to treatment before high stress
```{r}
set.seed(42)
compare2<-ps1
subs1 <- subset_samples(compare2, treatment_cycle3 %in% c("control_cycle_3_before_stress", "Chlorothalonil_cycle_3_before_stress"))
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle3,
       data = metadata, perm=999)

set.seed(42)
subs1 <- subset_samples(compare2, treatment_cycle3 %in% c("control_cycle_3_before_stress", "Glyphosate_cycle_3_before_stress"))
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle3,
       data = metadata, perm=999)

set.seed(42)
subs1 <- subset_samples(compare2, treatment_cycle3 %in% c("control_cycle_3_before_stress", "Tetracycline_cycle_3_before_stress"))
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle3,
       data = metadata, perm=999)
```
Before high stress application, Glyphosate and Tetracycline is significantly different to the controls. Chlorothalonil isn't

#### compare control to treatment after high stress

```{r}
set.seed(42)
subs1 <- subset_samples(compare1, treatment_cycle %in% c("control_Chlorothalonil_cycle_3_after_stress", "Chlorothalonil_cycle_3_after_stress"))
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle,
       data = metadata, perm=999)

subs1 <- subset_samples(compare1, treatment_cycle %in% c("control_Glyphosate_cycle_3_after_stress", "Glyphosate_cycle_3_after_stress"))
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle,
       data = metadata, perm=999)

subs1 <- subset_samples(compare1, treatment_cycle %in% c("control_Tetracycline_cycle_3_after_stress", "Tetracycline_cycle_3_after_stress"))
metadata <- as(sample_data(subs1), "data.frame")
adonis(distance(subs1, method="bray") ~ treatment_cycle,
       data = metadata, perm=999)
```
After high stress, Chlorothalonil and Glyphosate responded significantly different to the high stress in comparison to the respective control (control also stressed with the toxins!), For Tetracycline no difference (likely because the strong effect of Tetracycline is simply disturbing everything)
...
```{r}
test<-ps1
test = subset_samples(test, cycle !="positive_control" & cycle !="hive_bee"&cycle !="start_microbiome")
...
cycle3_a = subset_samples(test2, cycle == "cycle_three_after_stress")
cycle3_a <- prune_taxa(taxa_sums(cycle3_a) > 0, cycle3_a)
...
plot(betadisper(d_cycle1, groups))

boxplot(betadisper(d_cycle1, groups))
...
cycle1.1 = subset_samples(cycle1, treatment3 == "Tetracycline" | treatment3 == "control")
cycle1 <- prune_taxa(taxa_sums(cycle1) > 0, cycle1)

```

We see that Tetracycline is farer from all the other treatments. Plot it seperately for cycle 1 and 2

Result of betadisper is non-significant, so we can trust ADONIS output (spread within treatments is not higher than)
